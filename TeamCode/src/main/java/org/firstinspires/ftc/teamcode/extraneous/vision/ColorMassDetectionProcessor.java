package org.firstinspires.ftc.teamcode.extraneous.vision;
...
public class ColorMassDetectionProcessor implements VisionProcessor, CameraStreamSource {
    ...
    private final Paint targetSquarePaint;
    ...
    private final Rect targetSquare;
     ...
     */
    public ColorMassDetectionProcessor(@NonNull Scalar lower, @NonNull Scalar upper, DoubleSupplier minArea, DoubleSupplier left, DoubleSupplier right, Rect targetSquare) {
        ...
        // New paint for target square
        targetSquarePaint = new Paint();
        targetSquarePaint.setColor(Color.MAGENTA);
        targetSquarePaint.setStyle(Paint.Style.STROKE);
        targetSquarePaint.setStrokeWidth(8);
        targetSquarePaint.setAntiAlias(true);
    }
...
//		Imgproc.drawContours(frame, contours, -1, colour);
    ...
    public void onDrawFrame(Canvas canvas, int onscreenWidth, int onscreenHeight, float scaleBmpPxToCanvasPx, float scaleCanvasDensity, Object userContext) {
        ...
        float targetLeft = targetSquare.x * scaleBmpPxToCanvasPx;
        float targetTop = targetSquare.y * scaleBmpPxToCanvasPx;
        float targetRight = (targetSquare.x + targetSquare.width) * scaleBmpPxToCanvasPx;
        float targetBottom = (targetSquare.y + targetSquare.height) * scaleBmpPxToCanvasPx;
        canvas.drawRect(targetLeft, targetTop, targetRight, targetBottom, targetSquarePaint);
...
//		}
        ...
        if (largestContour != null) {
            ...
            float[] points = {
                    rect.x * scaleBmpPxToCanvasPx, rect.y * scaleBmpPxToCanvasPx,
                    (rect.x + rect.width) * scaleBmpPxToCanvasPx, (rect.y + rect.height) * scaleBmpPxToCanvasPx
            };
    ...
    public boolean isContourInsideTarget() {
        if (largestContour == null) return false;
        Point contourCenter = new Point(largestContourX, largestContourY);
        return targetSquare.contains(contourCenter);
    }
